CC Hello, here is the program for the 2D elastic halfspace with a pressure
CC  source and receivers above another elastic halfspace, with examples of
CC  input and output. Before use, please read carefully the
CC  intro-comments in the FORTRAN program (records 0483 to 0714).
CC  Numerical quadrature is performed by a routine from the NAG-library.
CC  The FORTRAN source code of the called NAG-routines (i.e. D01AHF and
CC  its auxiliaries) have been included. Please keep this part of the code
CC  confidential and for non-commercial use only. Thanks.
CC  The rest of this file contains:
CC  The indata file             (unit 5) from record 0023 to 0039
CC  A corresponding output file (unit 9) from record 0043 to 0074
CC  A corresponding output file (unit 8) from record 0078 to 0479
CC  The complete FORTRAN source code     from record 0483 to 3283
CC  with NAG D01AHF FORTRAN source code  from record 1694 to 3283



CC EXACT 2D RESPONSE FOR ELASTIC/ELASTIC INTERFACE, COMPRESSIONAL SOURCE
CC  TMIN         TMAX         NTSTP                      <-- TIME WINDOW
CC  0.000D0      0.400D0       200
CC  DELAY        FP           ZS                         <-- SOURCE
CC  0.050D0       20.00D0    47.50D0
CC  XREC         ZREC         DXR          DZR      NREC <-- RECEIVERS
CC  200.00D0     72.50D0     200.0D0     -50.0D0      2
CC  CP1          CS1          RHO1                       <-- MEDIUM I
CC  2200.0D0     1500.0D0     1.0D0
CC  CP2          CS2          RHO2                       <-- MEDIUM II
CC  2000.0D0     1200.0D0     1.5D0
CC  ITRACE(1)    ITRACE(2)    ITRACE(3)                  <-- P-WAVES
CC  1            1            1
CC               ITRACE(4)    ITRACE(5)                  <-- S-WAVES
CC               1            1
CC  OUTPUT(1)    OUTPUT(2)    DISPV        FAC           <-- OUTPUT
CC 'TAB'         '   '        1            1.000D+00
...
CC DT               =      2.000 MSEC
CC MEDIUM 1:         ELASTIC
CC     P VELOCITY   =   2200.000 M/SEC
CC     S VELOCITY   =   1500.000 M/SEC
CC     DENSITY      =      1.000 G/CM**3
CC MEDIUM 2:         ELASTIC
CC     P VELOCITY   =   2000.000 M/SEC
CC     S VELOCITY   =   1200.000 M/SEC
CC     DENSITY      =      1.500 G/CM**3
CC SOURCE HIGHT     =     47.500 M
CC SOURCE PEEK FREQ.=     20.000 HZ
CC SOURCE DELAY     =     50.000 MSEC
CC SOURCE AT T=0    =      0.000
CC TIME , DU/DT AND DW/DT ON UNIT 8

CC ****************** TRAVEL TIME SCHEME (IN MSEC) *******************

CCBODY WAVES: DIR-P IS DIRECT P-WAVE,
CC            PP IS PP-REFLECTION, AND
CC            PS IS PS-REFLECTION.
CCHEAD WAVES: HPP AND HSP ARE CONTRIBUTIONS TO
CC            PP-REFL FROM PRIMARY AND SECONDARY
CC            HEAD WAVE SYSTEMS, RESP, AND
CC            HPS AND HSS ARE CONTRIBUTIONS TO
CC            PS-REFL FROM PRIMARY AND SECONDARY
CC            HEAD WAVE SYSTEMS, RESP.

CCUNDEFINED ARRIVAL TIMES REPRESENTED BY -999.9

CC NO      XR      ZR  DIR-P     PP    HPP    HSP     PS    HPS    HSS
CC  1  200.00   72.50   91.6  106.0 -999.9 -999.9  129.9 -999.9 -999.9
CC  2  400.00   22.50  182.2  184.6 -999.9 -999.9  194.1 -999.9 -999.9
...
************************************************************************
*
*  THIS FORTRAN PROGRAM CALCULATES THE EXACT SEISMIC 2D RESPONSE IN AN
*  ELASTIC HALFSPACE WITH A COMPRESSIONAL POINT SOURCE AND AN INTERFACE
*  TO AN ELASTIC HALFSPACE. ALL GREEN'S FUNCTIONS ARE CALCULATED
*  ANALYTICALLY BY CAGNIARD-DE HOOP TECHNIQUE. CONVOLUTIONS WITH A GIVEN
*  SOURCE TIME HISTORY ARE PERFORMED NUMERICALLY.
*
*                      22222    DDDDD
*                     22   22   DD  DD
*                         22    DD   DD
...
************************************************************************
*
*  GEOMETRY: (SOURCE AND RECEIVERS ALWAYS ABOVE THE INTERFACE)
*
*         |
*         |
*   -ZS   *   SOURCE
...
*         |                                        *********************
*         |
*     0   +----------------------|------------ ELASTIC/ELASTIC INTERFACE
*         0                      XR
*                                                  ELASTIC HALFSPACE II:
*                                                    Z > 0
*                                                  *********************
*                                                  * P-VELOCITY = CP2  *
*                                                  * S-VELOCITY = CS2  *
...
*  PARAMETER VALUES:    CP1 AND RHO1 MUST BOTH BE POSITIVE.
*                       CP2 AND RHO2 MUST BOTH BE NON-NEGATIVE.
*                       0 LE CS1 LE CP1/SQRT(2).
*                       0 LE CS2 LE CP2/SQRT(2).
*                       POSSIBLE INTERFACES:
*                         ELASTIC/ELASTIC:   ALL SIX PARAMETERS GT 0.
*                         ACOUSTIC/ELASTIC:  CS1=0.
*                         ELASTIC/ACOUSTIC:  CS2=0.
*                         ACOUSTIC/ACOUSTIC: CS1=CS2=0.
*                         FREE SURFACE:      CP2=CS2=0 AND/OR RHO2=0,
*                                            BOTH ACOUSTIC (CS1=0) AND
*                                            ELASTIC (CS1>0).
*  Z-AXIS:              VERTICAL, POSITIVE DIRECTION DOWNWARDS.
*  X-AXIS:              HORIZONTAL, ALONG THE INTERFACE.
...
*                       OF GREEN'S FUNCTION AND S'(T) OR S''(T).
...
*
* 1: SOURCE (P-WAVE)                   -> RECEIVER (DIRECT WAVE)
* 2: SOURCE (P-WAVE)-> SURFACE (P-WAVE)-> RECEIVER (BODY WAVE)
* 3: SOURCE (P-WAVE)-> SURFACE (P-WAVE)-> RECEIVER (HEAD WAVES)  (*)
* 4: SOURCE (P-WAVE)-> SURFACE (S-WAVE)-> RECEIVER (BODY WAVE)
* 5: SOURCE (P-WAVE)-> SURFACE (S-WAVE)-> RECEIVER (HEAD WAVES)  (*)
*
* (*): THE HEAD WAVES MAY INCLUDE CONTRIBUTIONS BOTH FROM THE PRIMARY
*      HEAD WAVE SYSTEM AND FROM THE SECONDARY HEADWAVE SYSTEM.
*
************************************************************************
*
*    INPUT: UNIT 5      (EX2DELEL INDATA)
*
...
* REAL*8      ZS        SOURCE HIGHT                            (METERS)
* REAL*8      XREC      HORIZONTAL DISTANCE SOURCE-1'ST RECEIVER(METERS)
* REAL*8      ZREC      HIGHT OF FIRST RECEIVER                 (METERS)
* REAL*8      DXR       RECEIVER DISTANCE INCREMENT             (METERS)
* REAL*8      DZR       RECEIVER HIGHT INCREMENT                (METERS)
* INTEGER     NREC      NO.OF RECEIVERS SPACED BY DXR,DZR
* REAL*8      CP1       P-VELOCITY, MEDIUM I                (METERS/SEC)
* REAL*8      CS1       S-VELOCITY, MEDIUM I                (METERS/SEC)
* REAL*8      RHO1      DENSITY, MEDIUM I                      (G/CM**3)
* REAL*8      CP2       P-VELOCITY, MEDIUM II               (METERS/SEC)
* REAL*8      CS2       S-VELOCITY, MEDIUM II               (METERS/SEC)
* REAL*8      RHO2      DENSITY, MEDIUM II                     (G/CM**3)
...
* INTEGER     ITRACE(2) IF 1 INCLUDE P-P REFLECTION, BODY WAVE PART
* INTEGER     ITRACE(3) IF 1 INCLUDE P-P REFLECTION, HEAD WAVE PART
* INTEGER     ITRACE(4) IF 1 INCLUDE P-S REFLECTION, BODY WAVE PART
* INTEGER     ITRACE(5) IF 1 INCLUDE P-S REFLECTION, HEAD WAVE PART
*
* NOTE:       FOR QUICK OVERVIEW SET ITRACE(I)=0 FOR ALL I=1,2,...,5 ,
*             THEN ONLY OUTPUT OF ARRIVAL TIMES, CRITICAL DISTANCES ETC.
*
************************************************************************
*
*    OUTPUT: UNIT 9  (EX2DELEL SYSPRINT)
*
*      ERROR MESSAGES
...
************************************************************************
*
*    OUTPUT: UNIT 8  (EX2DELEL OUTDATA)
*
...
************************************************************************
*
*    OUTPUT: UNIT 10 (EX2DELEL HORIDISP)
*
...
************************************************************************
*
*    OUTPUT: UNIT 11 (EX2DELEL VERTDISP)
*
*      IF OUTPUT(1) OR OUTPUT(2) = 'CON'
...
************************************************************************
      ...
      REAL*8      U(0:NN),W(0:NN),T(0:NN),S
      REAL*8      PR,PPS,A1,A2,DELAY,FP
      REAL*8      TP,TPP,TPS,THPP,THSP,THPS,THSS,TEND
      REAL*8      TMIN,TMAX,DT,X,ZR,ZS,HEADD,Z,R,R2,TWOPI
      REAL*8      CP12,CS12,CP22,CS22,CP1,CS1,CP2,CS2,RHO1,RHO2,RHO
      REAL*8      YL,YH,TIME,XR,C2,TARRIV,USIGN,CP12I,CS12I
      REAL*8      ZERO,HALF,ONE,TWO,EPS,FAC,UNDEF
      REAL*8      XREC,ZREC,DXR,DZR,MSEC,RELE
      REAL*8      D01AHF,FINDP
      REAL*8      TRAC1U,TRAC2U,TRACPU,TRAC5U,TRA05U,TRACSU,TRA0SU
      REAL*8      TRAC1W,TRAC2W,TRACPW,TRAC5W,TRA05W,TRACSW,TRA0SW
      REAL*8      realTP,realTPP,realTHPP,realTHSP,realTPS
	REAL*8	realTHPS,realTHSS
...
c *** fin ajout D.K.
      ...
      INTEGER     IFAIL,NPTS
      INTEGER     ITRACE(5),DISPV,NREC
      CHARACTER*1 ADUM
      CHARACTER*3 OUTPUT(2)
      CHARACTER*8 TXT
      LOGICAL     OUTP
      EXTERNAL    FINDP,
     &            TRAC1U,TRAC2U,TRACPU,TRAC5U,TRA05U,TRACSU,TRA0SU,
     &            TRAC1W,TRAC2W,TRACPW,TRAC5W,TRA05W,TRACSW,TRA0SW
      COMMON /PARAM/  CS1,CP2,CS2,RHO
      COMMON /PARAM2/ CP12,CS12,CP22,CS22
      COMMON /PARAM3/ CP12I,CS12I
      COMMON /SOURCE/ A1,A2,DELAY,DISPV
      COMMON /NAG1/   X,Z,C2,R2,TIME,TARRIV
      COMMON /NAG2/   ZR,ZS,PPS,PC
      DATA TWOPI      /6.2831853071795865D+00/
      DATA EPS,MSEC,UNDEF   /1.00D-08,1.00D+03,-0.9999D+00/
      DATA ZERO,HALF,ONE,TWO /0.00D+00,0.50D+00,1.00D+00,2.00D+00/
      OPEN(5 ,FILE='INDATA_BICOUCHE',FORM='FORMATTED')
...
*************** READ INDATA ON UNIT 5
      ...
      READ(5,'(A1)') ADUM
      READ(5,*     )           ITRACE(4),ITRACE(5)
      READ(5,'(A1)') ADUM
      READ(5,*     ) OUTPUT(1),OUTPUT(2),DISPV,FAC

*************** CHECK INPUT PARAMETERS
      IF(ZS.GT.ZERO) GOTO 2
        WRITE(9,*) 'ZS MUST BE POSITIVE.'
        STOP
    2 IF(ZREC.GE.ZERO) GOTO 3
        WRITE(9,*) 'ZREC MUST NOT BE NEGATIVE.'
        STOP
    3 IF((NTSTP.GT.0).AND.(NTSTP.LE.NN)) GOTO 4
        WRITE(9,*) 'NTSTP MUST BE POSITIVE AND LESS THAN',NN+1
        STOP
    4 IF(TMIN.LT.TMAX) GOTO 5
        WRITE(9,*) 'TMIN MUST BE LESS THAN TMAX.'
        STOP
    5 IF((CS1.GE.ZERO).AND.(CS2.GE.ZERO)) GOTO 6
        WRITE(9,*) 'SHEAR VELOCITIES MUST NOT BE NEGATIVE.'
        STOP
    6 IF(DSQRT(TWO)*CS1.LT.CP1) GOTO 7
        WRITE(9,*) 'CP1 MUST BE GREATER THAN SQRT(2)*CS1.'
        STOP
    7 IF(CP2.EQ.ZERO.OR.DSQRT(TWO)*CS2.LT.CP2) GOTO 8
        WRITE(9,*) 'CP2 MUST BE GREATER THAN SQRT(2)*CS2.'
        STOP
    8 IF(RHO1.GT.ZERO) GOTO 9
        WRITE(9,*) 'DENSITY RHO1 MUST BE POSITIVE.'
        STOP
    9 IF(RHO2.GE.ZERO) GOTO 10
        WRITE(9,*) 'DENSITY RHO2 MUST NOT BE NEGATIVE.'
        STOP
   10 CONTINUE
*************** SET PARAMETERS
      OUTP = .FALSE.
      ...
      CP12I = ONE/CP12
      CS12I = UNDEF
      IF(CS1.GT.ZERO) CS12I = ONE/CS12
      IF(CP2.EQ.ZERO.OR.RHO2.EQ.ZERO) THEN
        CP2  = ZERO
        CS2  = ZERO
        RHO2 = ZERO
      ENDIF
...
***************  WRITE RUN INF.ON UNIT 9
      ...
      TXT = 'ELASTIC '
      IF(CS1.EQ.ZERO) TXT='ACOUSTIC'
      WRITE(9,'(A19      ,A8)')' MEDIUM 1:         ',TXT
      WRITE(9,'(A19,F11.3,A6)')'     P VELOCITY   =',CP1  ,' M/SEC'
      WRITE(9,'(A19,F11.3,A6)')'     S VELOCITY   =',CS1  ,' M/SEC'
      WRITE(9,'(A19,F11.3,A8)')'     DENSITY      =',RHO1 ,' G/CM**3'
      TXT = 'ELASTIC '
      IF(CS2.EQ.ZERO) TXT='ACOUSTIC'
      IF(CP2.EQ.ZERO) TXT='VACUUM  '
      WRITE(9,'(A19      ,A8)')' MEDIUM 2:         ',TXT
      WRITE(9,'(A19,F11.3,A6)')'     P VELOCITY   =',CP2  ,' M/SEC'
      WRITE(9,'(A19,F11.3,A6)')'     S VELOCITY   =',CS2  ,' M/SEC'
      WRITE(9,'(A19,F11.3,A8)')'     DENSITY      =',RHO2 ,' G/CM**3'
      WRITE(9,'(A19,F11.3,A5)')' SOURCE HIGHT     =',ZS   ,' M   '
  ...
  1   CONTINUE
     ...
     &//'TRAVEL TIME SCHEME (IN MSEC) *******************'
      WRITE(9,'(A1 )')' '
      WRITE(9,'(A  )')'BODY WAVES: DIR-P IS DIRECT P-WAVE,'
      WRITE(9,'(A  )')'            PP IS PP-REFLECTION, AND'
      WRITE(9,'(A  )')'            PS IS PS-REFLECTION.'
      WRITE(9,'(A  )')'HEAD WAVES: HPP AND HSP ARE CONTRIBUTIONS TO'
      WRITE(9,'(A  )')'            PP-REFL FROM PRIMARY AND SECONDARY'
      WRITE(9,'(A  )')'            HEAD WAVE SYSTEMS, RESP, AND'
      WRITE(9,'(A  )')'            HPS AND HSS ARE CONTRIBUTIONS TO'
      WRITE(9,'(A  )')'            PS-REFL FROM PRIMARY AND SECONDARY'
      WRITE(9,'(A  )')'            HEAD WAVE SYSTEMS, RESP.'
      WRITE(9,'(A1 )')' '
      WRITE(9,'(A  )')'UNDEFINED ARRIVAL TIMES REPRESENTED BY -999.9'
      WRITE(9,'(A1 )')' '
      WRITE(9,'(A68)')
     &'NO      XR      ZR'
     &//'  DIR-P     PP    HPP    HSP     PS    HPS    HSS'
*
***************  START RECEIVER LOOP
*
      write(6,*) 'nb total de recepteurs ',NREC
      DO 1000 IR = 0,NREC-1
...
*
       THPP = UNDEF
       IF(CP2.GT.CP1.AND.XR.GT.(ZR+ZS)/DSQRT(CP22/CP12-ONE)) THEN
         THPP = XR/CP2 + (ZS+ZR)*DSQRT(ONE/CP12-ONE/CP22)
       ENDIF
*
       THPS = UNDEF
       IF(CP2.GT.CP1.AND.CS1.GT.ZERO.AND.XR.GT.
     &    ZS/DSQRT(CP22/CP12-ONE)+ZR/DSQRT(CP22/CS12-ONE)) THEN
         THPS = XR/CP2 + ZS*DSQRT(ONE/CP12-ONE/CP22)
     &                 + ZR*DSQRT(ONE/CS12-ONE/CP22)
       ENDIF
*
       THSP = UNDEF
       IF(CS2.GT.CP1.AND.XR.GT.(ZS+ZR)/DSQRT(CS22/CP12-ONE)) THEN
         THSP = XR/CS2 + (ZS+ZR)*DSQRT(ONE/CP12-ONE/CS22)
       ENDIF
*
       THSS = UNDEF
       IF(CS2.GT.CP1.AND.CS1.GT.ZERO.AND.XR.GT.
     &    ZS/DSQRT(CS22/CP12-ONE)+ZR/DSQRT(CS22/CS12-ONE)) THEN
         THSS = XR/CS2 + ZS*DSQRT(ONE/CP12-ONE/CS22)
     &                 + ZR*DSQRT(ONE/CS12-ONE/CS22)
       ENDIF
*
       PR  = HALF/CP1
       PPS = ZERO
       IF(ZR.GT.ZERO.AND.CS1.GT.ZERO) THEN
         IF(DABS(XR).GE.EPS) PPS = FINDP(PR,XR,ZR,ZS)
         TPS = XR*PPS
     &        +ZS*DSQRT(ONE/CP12-PPS**2)+ZR*DSQRT(ONE/CS12-PPS**2)
       ELSE
         IF(CS1.GT.ZERO) THEN
           PPS = XR/(CP1*DSQRT(XR*XR+ZS*ZS))
           TPS = TP
         ELSE
           PPS = UNDEF
           TPS = UNDEF
       ...
       ENDIF
*
c****** D.K. ajout du delay pour arrival times
	realTP = TP*MSEC 
	realTPP = TPP*MSEC 
	realTHPP = THPP*MSEC 
	realTHSP = THSP*MSEC 
	...
	realTHPS = THPS*MSEC 
	realTHSS = THSS*MSEC 
	if(TP.gt.0.d0) realTP = realTP + delay*1.d3
	if(TPP.gt.0.d0) realTPP = realTPP + delay*1.d3
	if(THPP.gt.0.d0) realTHPP = realTHPP + delay*1.d3
	if(THSP.gt.0.d0) realTHSP = realTHSP + delay*1.d3
	if(TPS.gt.0.d0) realTPS = realTPS + delay*1.d3
	if(THPS.gt.0.d0) realTHPS = realTHPS + delay*1.d3
	if(THSS.gt.0.d0) realTHSS = realTHSS + delay*1.d3

       WRITE(9,'(I3,2(F8.2),7(F7.1))')
     &  IR+1,XR,ZR,realTP,realTPP,realTHPP,
     &  realTHSP,realTPS,realTHPS,realTHSS

c       WRITE(9,*) 'old ci-dessous'
c       WRITE(9,'(I3,2(F8.2),7(F7.1))')
c     &  IR+1,XR,ZR,TP*MSEC,TPP*MSEC,THPP*MSEC,
c     &  THSP*MSEC,TPS*MSEC,THPS*MSEC,THSS*MSEC
...
*****************************************************************
 ...
 101    YH    = TIME/TP
        ...
        IFAIL = 0
        U(NT) =       D01AHF(YL,YH,EPSR,NPTS,RELE,TRAC1U,0,IFAIL)
        IFAIL = 0
        W(NT) = USIGN*D01AHF(YL,YH,EPSR,NPTS,RELE,TRAC1W,0,IFAIL)
 100  CONTINUE
*
 299  CONTINUE
      ...
      OUTP = .TRUE.
*****************************************************************
*        PP REFLECTED BODY WAVE  (TPP)                    TRACE 2
...
*
 399  CONTINUE
      ...
      OUTP = .TRUE.
*****************************************************************
*        PP REFLECTED HEAD WAVES         (THPP)           TRACE 3
*        USE TRANS: TAU = TPP*SIN(Y) , THPP < TAU < TPP
*****************************************************************
      IF(CP2.LE.CP1) GOTO 499
      Z     = ZR+ZS
      R2    = XR*XR+Z*Z
      X     = XR
      TARRIV= TPP
      C2    = CP12
      HEADD = Z/DSQRT( CP22/CP12-ONE )
      IF(X.LE.HEADD) GOTO 499
      TEND  = TPP
      NTMIN = MAX0(0,IDINT((THPP-TMIN)/DT))
      DO 300 NT=NTMIN,NTSTP
       TIME  = T(NT)
       IF(TIME.LE.THPP) GOTO 300
       IF(TIME-TWO*DELAY.GE.TEND) GOTO 300
        YL    = DMAX1(TIME-TWO*DELAY,THPP)
        YH    = DMIN1(TIME,TEND)
        YL    = DASIN(YL/TPP)
        YH    = DASIN(YH/TPP)
        IFAIL = 0
        U(NT) = U(NT)+D01AHF(YL,YH,EPSR,NPTS,RELE,TRACPU,0,IFAIL)
        IFAIL = 0
        W(NT) = W(NT)+D01AHF(YL,YH,EPSR,NPTS,RELE,TRACPW,0,IFAIL)
 300  CONTINUE
*
 499  CONTINUE
      ...
      OUTP = .TRUE.
*****************************************************************
*        PS REFLECTED BODY WAVE (TPS)                     TRACE 4
*        USE TRANS: TAU = TPS*DCOSH(Y) IF ZR=0
*****************************************************************
      IF(CS1.LE.ZERO) GOTO 599
        X     = XR
        TARRIV= TPS
        C2    = CS12
      IF(ZR.GT.ZERO) THEN
 ...
 451      YH    = TIME/TPS
          YH    = DLOG(YH+DSQRT(YH*YH-ONE))
          IFAIL = 0
          U(NT) = U(NT)+D01AHF(YL,YH,EPSR,NPTS,RELE,TRA05U,0,IFAIL)
          IFAIL = 0
          W(NT) = W(NT)+D01AHF(YL,YH,EPSR,NPTS,RELE,TRA05W,0,IFAIL)
 450    CONTINUE
      ENDIF
*
 599  CONTINUE
      ...
      OUTP = .TRUE.
*****************************************************************
*        PS REFLECTED HEAD WAVES         (THPS)           TRACE 5
*        USE TRANS: TAU = TPS*SIN(Y) , THPS < TAU < TPS
*                   IF ZR=0
*****************************************************************
      IF(CP2.LE.CP1.OR.CS1.LE.ZERO) GOTO 699
        X     = XR
        TARRIV= TPS
        C2    = CS12
        HEADD = ZS/DSQRT( CP22/CP12-ONE )+ZR/DSQRT( CP22/CS12-ONE )
      IF(X.LE.HEADD) GOTO 699
      TEND  = TPS
      NTMIN = MAX0(0,IDINT((THPS-TMIN)/DT))
      IF(ZR.GT.ZERO) THEN
        Z     = ZR+ZS
        R2    = XR*XR+Z*Z
        DO 500 NT=NTMIN,NTSTP
         TIME  = T(NT)
         IF(TIME.LE.THPS) GOTO 500
         IF(TIME-TWO*DELAY.GE.TEND) GOTO 500
           YL    = DMAX1(TIME-TWO*DELAY,THPS)
           YH    = DMIN1(TIME,TEND)
           IFAIL = 0
           PC    = DCMPLX(ONE/CP2,ZERO)
           U(NT) = U(NT)+D01AHF(YL,YH,EPSR,NPTS,RELE,TRACSU,0,IFAIL)
           IFAIL = 0
           PC    = DCMPLX(ONE/CP2,ZERO)
           W(NT) = W(NT)+D01AHF(YL,YH,EPSR,NPTS,RELE,TRACSW,0,IFAIL)
 500    CONTINUE
      ELSE
        Z     = ZS
        R2    = XR*XR+Z*Z
        DO 550 NT=NTMIN,NTSTP
         TIME  = T(NT)
         IF(TIME.LE.THPS) GOTO 550
         IF(TIME-TWO*DELAY.GE.TEND) GOTO 550
           YL    = DMAX1(TIME-TWO*DELAY,THPS)
           YH    = DMIN1(TIME,TEND)
           YL    = DASIN(YL/TPS)
           YH    = DASIN(YH/TPS)
           IFAIL = 0
           U(NT) = U(NT)+D01AHF(YL,YH,EPSR,NPTS,RELE,TRA0SU,0,IFAIL)
           IFAIL = 0
           W(NT) = W(NT)+D01AHF(YL,YH,EPSR,NPTS,RELE,TRA0SW,0,IFAIL)
 550    CONTINUE
      ENDIF
*
*
...
*
*************** WRITE TIME AND U AND W DISP ON UNIT 8
       DO 801 J=1,2
       IF(OUTPUT(J).EQ.'TAB')
     &  WRITE(8,850)(T(I),U(I)*FAC,W(I)*FAC,I=0,NTSTP)
*
 801   CONTINUE
...
c *** modif D.K. ecriture sismogrammes normalises et au format SEP
      ...
      do it=1,ntstp
c DK DK DK inversion du signe pour compatibilite convention supprimee
c et conversion en simple precision
c DK DK DK decalage de 1 timestep pour calage origine en temps
	if(it.gt.1) then
	      horiz(it,ir+1) = sngl(U(it-1))
	      vert(it,ir+1) = sngl(W(it-1))
	else
 ...
 1000 CONTINUE
 850  FORMAT(3(1PE16.5))
 855  FORMAT(1(1PE16.5))
 860  FORMAT(7(1PE11.3))
...
c ASCII output (by M. Käser)
	...
	WRITE(9,*) ' '
	WRITE(9,*) ' WITH SOURCE DELAY INCLUDED IN THE ARRIVAL TIME'
	CLOSE(9)
...
*     FOR A REAL P < 1/CP.                                             *
************************************************************************
      REAL FUNCTION FINDP*8(P,X,Z1,Z2)
      REAL*8 P,P1,GP,GS,DFDP,FAP
      REAL*8 CP2I,CS2I,X,Z1,Z2,FEPS,CPF,TWO,TEMP
      COMMON /PARAM3/ CP2I,CS2I
...
************************************************************************
      COMPLEX FUNCTION RP*16(P2,GP1)
      COMPLEX*16 P2,GP1,GS1,GP2,GS2,D2,D,C
      REAL*8     CS1,CP2,CS2,RHO,CP12,CS12,CP22,CS22
      REAL*8     ZERO,ONE,TWO,FOUR,Q
      PARAMETER  (ZERO=0.D0,ONE=1.D0,TWO=2.D0,FOUR=4.D0)
      COMMON /PARAM/  CS1,CP2,CS2,RHO
      COMMON /PARAM2/ CP12,CS12,CP22,CS22
      GS1 = CDSQRT(ONE-CS12*P2)
      GP2 = CDSQRT(ONE-CP22*P2)
      IF(CS2.GT.ZERO) THEN
        GS2 = CDSQRT(ONE-CS22*P2)
        D2  = CP2*CS2*P2+GP2*GS2
        Q   = TWO*(CS12-RHO*CS22)
        D   = RHO+TWO*Q*P2
        C   = CS2*RHO*( P2*(D-TWO)*CS1*CP2+GS1*GP2 )
     &      + CS1*D2*( (ONE-Q*P2)**2 )
        D   = GP1*( CP2*RHO*(GS1*D*CS2+GS2*CS1)+D2*GS1*Q*Q*P2 )
      ELSE
        C   = GP2*( (ONE-TWO*CS12*P2)**2 )
        D   = GP1*( RHO*CP2+FOUR*GS1*GP2*CS12*CS1*P2 )
      ENDIF
      RP  = (D-C)/(D+C)
      RETURN
      END
*
************************************************************************
*     PS REFLECTION COEFFICIENT DIVIDED BY (P*GP1*CS1)                 *
************************************************************************
      COMPLEX FUNCTION RS*16(P2,GS1)
      COMPLEX*16 P2,GP1,GS1,GP2,GS2,D2,D,C
      REAL*8     CS1,CP2,CS2,RHO,CP12,CS12,CP22,CS22
      REAL*8     ZERO,ONE,TWO,FOUR,Q
      PARAMETER  (ZERO=0.D0,ONE=1.D0,TWO=2.D0,FOUR=4.D0)
      ...
      IF(CS2.GT.ZERO) THEN
        ...
        C   = CS2*RHO*( P2*(D-TWO)*CS1*CP2+GS1*GP2 )
     &      + CS1*D2*( (ONE-Q*P2)**2 )
     &      + GP1*( CP2*RHO*(GS1*D*CS2+GS2*CS1)+D2*GS1*Q*Q*P2 )
        D   = RHO*(ONE-RHO)*CP2*CS2-Q*( P2*(Q*D2+TWO*RHO*CP2*CS2)-D2 )
      ELSE
        C   = GP2*( (ONE-TWO*CS12*P2)**2 )
     &      + GP1*( RHO*CP2+FOUR*GS1*GP2*CS12*CS1*P2 )
        D   = TWO*CS1*GP2*( ONE-TWO*CS12*P2 )
      ENDIF
      RS  = TWO*D/C
      RETURN
      END
...
************************************************************************
      COMPLEX FUNCTION PTAU*16(P,DPDT,TAU,X,Z1,Z2,PPS)
      COMPLEX*16 P,P1,GP,GS,DPDT,FAP
      REAL*8 CP2I,CS2I,TAU,X,Z1,Z2,FEPS,PI,ZERO,ONE,EPS,FAPR,FAPI,PR,PPS
      COMMON /PARAM3/ CP2I,CS2I
...
*
************************************************************************
*     NUMERICAL COMPUTATION OF THE CAGNIARD-CONTOUR FOR HEAD WAVES:    *
*         P*X + GAMMAS*Z1 + GAMMAP*Z2 = TAU > 0 ,                      *
*     I.E. FOR THPS < TAU < TPS A REAL SOLUTION P(TAU) IS FOUND, WHERE *
*         0 < 1/CP2 < P(TAU) < PPS < 1/CP1 ,                           *
*     AS WELL AS A REAL DERIVATIVE                                     *
*         DP/DTAU = (X - P*Z1/GAMMAS - P*Z2/GAMMAP)**(-1)              *
************************************************************************
      REAL FUNCTION RPTAU*8(P,DPDT,TAU,X,Z1,Z2,PPS)
      REAL*8 P,P1,GP,GS,DPDT,FAP
      REAL*8 CP2I,CS2I,TAU,X,Z1,Z2,FEPS,HALF,ONE,EPS,PPS,P0
      COMMON /PARAM3/ CP2I,CS2I
      DATA HALF,ONE,FEPS,EPS /0.50D+00,1.0D+00,1.0D-12,1.0D-6/
      I=0
      P0   = P
      P1   = P
      DPDT = P
 ...
 10   CONTINUE
         ...
         FAP  = P1*X+Z1*GS+Z2*GP-TAU
         P    = P1-FAP*GP*GS/( X*GP*GS-P1*(Z1*GP+Z2*GS) )
         IF(P.GE.P0.AND.P.LE.PPS) GOTO 12
           P = HALF*(P0+PPS)
           GOTO 10
  12     CONTINUE
         IF(DABS(P-P1).GT.EPS*DABS(P+P1).OR.DABS(FAP).GT.FEPS) GOTO 10
      RPTAU = P
      GP    = DSQRT(CP2I-P*P)
      GS    = DSQRT(CS2I-P*P)
      DPDT  = GP*GS/(X*GP*GS-P*(Z1*GP+Z2*GS))
      RETURN
 9999 WRITE(9,*) '************** ERROR IN RPTAU ****************'
      WRITE(9,*) 'TAU,I=            ',TAU,I
      WRITE(9,*) 'P(I)-P(I-1):      ',P-P1
      WRITE(9,*) 'P(I)+P(I-1):      ',P+P1
      WRITE(9,*) 'FUNCTION VALUE:   ',FAP
      WRITE(9,*) 'FUNCTION DERIV:   ',ONE/( X-P*(Z1/GS+Z2/GP) )
      WRITE(9,*) 'P(I)              ',P
...
************************************************************************
      REAL FUNCTION TRAC1U*8(Y)
      INTEGER DISPV
      REAL*8 TAU,X,Z,C2,R2,TIME,SOUR,A1,A2,DELAY,TARRIV,Y
      COMMON /SOURCE/ A1,A2,DELAY,DISPV
      COMMON /NAG1/   X,Z,C2,R2,TIME,TARRIV
      EXTERNAL SOUR
      TAU    = TARRIV*DCOSH(Y)
      TRAC1U = X*TAU*SOUR(TIME-TAU)/R2
      RETURN
      END
******-------------------------*****************************************
      REAL FUNCTION TRAC1W*8(Y)
      INTEGER DISPV
      REAL*8 TAU,X,Z,C2,R2,TIME,SOUR,A1,A2,DELAY,TARRIV,Y
      COMMON /SOURCE/ A1,A2,DELAY,DISPV
      COMMON /NAG1/   X,Z,C2,R2,TIME,TARRIV
...
************************************************************************
      REAL FUNCTION TRAC2U*8(Y)
      INTEGER DISPV
      REAL*8 TAU,TIME,SOUR,A1,A2,DELAY,TARRIV,Y
      REAL*8 X,Z,R2,C2,CH,SH,TEMP
      COMPLEX*16 RP,P,P2,GAMP,CI/(0.0D+00,1.0D+00)/
      COMMON /SOURCE/ A1,A2,DELAY,DISPV
      COMMON /NAG1/   X,Z,C2,R2,TIME,TARRIV
      EXTERNAL SOUR,RP
      CH     = DCOSH(Y)
      ...
      P      = (X*CH - CI*Z*SH)/TEMP
      P2     = P*P
      GAMP   = (Z*CH + CI*X*SH)/TEMP
      TRAC2U = DREAL( P*RP(P2,GAMP) )*SOUR(TIME-TAU)
      RETURN
      END
******-------------------------*****************************************
      REAL FUNCTION TRAC2W*8(Y)
      INTEGER DISPV
      REAL*8 TAU,TIME,SOUR,A1,A2,DELAY,TARRIV,Y
      REAL*8 X,Z,R2,C2,CH,SH,TEMP
      COMPLEX*16 RP,P,P2,GAMP,CI/(0.0D+00,1.0D+00)/
      ...
      P2     = P*P
      GAMP   = (Z*CH + CI*X*SH)/TEMP
      TRAC2W = DREAL( -GAMP*RP(P2,GAMP) )*SOUR(TIME-TAU)
      RETURN
      END
*
************************************************************************
*     PP-REFL. HEAD WAVES: TRACPU & TRACPW                             *
************************************************************************
      REAL FUNCTION TRACPU*8(Y)
      INTEGER DISPV
      REAL*8 TAU,TIME,SOUR,A1,A2,DELAY,TARRIV,Y
      REAL*8 X,Z,R2,C2,CH,SH,TEMP,P,P2,GAMP,ZERO
      PARAMETER (ZERO=0.D0)
      COMPLEX*16 RP,ZGAMP,ZP2
      COMMON /SOURCE/ A1,A2,DELAY,DISPV
      COMMON /NAG1/   X,Z,C2,R2,TIME,TARRIV
      EXTERNAL SOUR,RP
      CH     = DCOS(Y)
      ...
      GAMP   = (X*CH + Z*SH)/TEMP
      ZP2    = DCMPLX(P2,ZERO)
      ZGAMP  = DCMPLX(GAMP,ZERO)
      TRACPU =-P*DIMAG( RP(ZP2,ZGAMP) )*SOUR(TIME-TAU)
      RETURN
      END
******-------------------------*****************************************
      REAL FUNCTION TRACPW*8(Y)
      INTEGER DISPV
      REAL*8 TAU,TIME,SOUR,A1,A2,DELAY,TARRIV,Y
      REAL*8 X,Z,R2,C2,CH,SH,TEMP,P,P2,GAMP,ZERO
      PARAMETER (ZERO=0.D0)
      COMPLEX*16 RP,ZP2,ZGAMP
      COMMON /SOURCE/ A1,A2,DELAY,DISPV
      COMMON /NAG1/   X,Z,C2,R2,TIME,TARRIV
      EXTERNAL SOUR,RP
      CH     = DCOS(Y)
      ...
      ZP2    = DCMPLX(P2,ZERO)
      ZGAMP  = DCMPLX(GAMP,ZERO)
      TRACPW = GAMP*DIMAG( RP(ZP2,ZGAMP) )*SOUR(TIME-TAU)
      RETURN
      END
*
************************************************************************
*     PS-REFL.: TRAC5U & TRAC5W FOR ZR > 0, CS1 > 0 IS ASSUMED
************************************************************************
      REAL FUNCTION TRAC5U*8(TAU)
      INTEGER DISPV
      REAL*8 TAU,TIME,SOUR,A1,A2,DELAY,X,ZR,ZS,PPS
      REAL*8 TARRIV,Z,C2,R2,ONE
      PARAMETER (ONE=1.D0)
      COMPLEX*16 P,P2,DPDT,PTAU,RS,PC,GAMS
      COMMON /SOURCE/ A1,A2,DELAY,DISPV
      COMMON /NAG1/   X,Z,C2,R2,TIME,TARRIV
      COMMON /NAG2/   ZR,ZS,PPS,PC
      EXTERNAL PTAU,RS,SOUR
      P      = PTAU(PC,DPDT,TAU,X,ZR,ZS,PPS)
      P2     = P*P
      GAMS   = CDSQRT(ONE-P2*C2)
      TRAC5U = -DIMAG( P*GAMS*RS(P2,GAMS)*DPDT )*SOUR(TIME-TAU)
      RETURN
      END
******---------------------------***************************************
      REAL FUNCTION TRAC5W*8(TAU)
      INTEGER DISPV
      REAL*8 TAU,TIME,SOUR,A1,A2,DELAY,X,ZR,ZS,PPS
      REAL*8 TARRIV,Z,C2,R2,ONE
      PARAMETER (ONE=1.D0)
      ...
      P2     = P*P
      GAMS   = CDSQRT(ONE-P2*C2)
      TRAC5W = -DSQRT(C2)*DIMAG( P2*RS(P2,GAMS)*DPDT )*SOUR(TIME-TAU)
      RETURN
      END
*
************************************************************************
*     PS-REFL.: TRA05U & TRA05W FOR ZR = 0, CS1 > 0 IS ASSUMED
************************************************************************
      REAL FUNCTION TRA05U*8(Y)
      INTEGER DISPV
      REAL*8 TAU,TIME,SOUR,A1,A2,DELAY,TARRIV,Y
      REAL*8 X,Z,R2,C2,CH,SH,TEMP,CP12,CS12,CP22,CS22,ONE/1.D0/
      COMPLEX*16 RS,P,P2,GAMS,GAMP,CI/(0.0D+00,1.0D+00)/
      COMMON /SOURCE/ A1,A2,DELAY,DISPV
      COMMON /PARAM2/ CP12,CS12,CP22,CS22
      COMMON /NAG1/   X,Z,C2,R2,TIME,TARRIV
      EXTERNAL SOUR,RS
      ...
      P2     = P*P
      GAMP   = (Z*CH + CI*X*SH)/TEMP
      GAMS   = CDSQRT(ONE-P2*CS12)
      TRA05U = DREAL( GAMP*P*GAMS*RS(P2,GAMS) )*SOUR(TIME-TAU)
      RETURN
      END
******-------------------------*****************************************
      REAL FUNCTION TRA05W*8(Y)
      INTEGER DISPV
      REAL*8 TAU,TIME,SOUR,A1,A2,DELAY,TARRIV,Y
      REAL*8 X,Z,R2,C2,CH,SH,TEMP,CP12,CS12,CP22,CS22,ONE/1.D0/
      COMPLEX*16 RS,P,P2,GAMS,GAMP,CI/(0.0D+00,1.0D+00)/
      ...
      GAMP   = (Z*CH + CI*X*SH)/TEMP
      GAMS   = CDSQRT(ONE-P2*CS12)
      TRA05W = DSQRT(CS12)*DREAL( GAMP*P2*RS(P2,GAMS) )*SOUR(TIME-TAU)
      RETURN
      END
*
************************************************************************
*     PS-REFL. HEAD WAVES: TRACSU & TRACSW FOR ZR > 0, CS1 > 0 ASSUMED
************************************************************************
      REAL FUNCTION TRACSU*8(TAU)
      INTEGER DISPV
      REAL*8 TAU,TIME,SOUR,A1,A2,DELAY,X,ZR,ZS,PPS
      REAL*8 TARRIV,Z,C2,R2,ONE/1.D0/,ZERO/0.D0/
      REAL*8 P,P2,DPDT,RPTAU,PR,GAMS
      COMPLEX*16 RS,ZP2,PC,ZGAMS
      COMMON /SOURCE/ A1,A2,DELAY,DISPV
      COMMON /NAG1/   X,Z,C2,R2,TIME,TARRIV
      COMMON /NAG2/   ZR,ZS,PPS,PC
      EXTERNAL RPTAU,RS,SOUR
      PR     = DREAL(PC)
      P      = RPTAU(PR,DPDT,TAU,X,ZR,ZS,PPS)
      P2     = P*P
      GAMS   = DSQRT(ONE-P2*C2)
      ZP2    = DCMPLX(P2,ZERO)
      ZGAMS  = DCMPLX(GAMS,ZERO)
      TRACSU = -P*GAMS*DPDT*DIMAG( RS(ZP2,ZGAMS) )*SOUR(TIME-TAU)
      RETURN
      END
******---------------------------***************************************
      REAL FUNCTION TRACSW*8(TAU)
      INTEGER DISPV
      REAL*8 TAU,TIME,SOUR,A1,A2,DELAY,X,ZR,ZS,PPS
      REAL*8 TARRIV,Z,C2,R2,ONE/1.D0/,ZERO/0.D0/
      REAL*8 P,P2,DPDT,RPTAU,PR,GAMS
      ...
      ZP2    = DCMPLX(P2,ZERO)
      ZGAMS  = DCMPLX(GAMS,ZERO)
      TRACSW = -DSQRT(C2)*P2*DPDT*DIMAG( RS(ZP2,ZGAMS) )*SOUR(TIME-TAU)
      RETURN
      END
*
************************************************************************
*     PS-REFL. HEAD WAVES: TRA0SU & TRAC0W FOR ZR = 0, CS1 > 0 ASSUMED
************************************************************************
      REAL FUNCTION TRA0SU*8(Y)
      INTEGER DISPV
      REAL*8 TAU,TIME,SOUR,A1,A2,DELAY,X,Y,CH,SH,TEMP
      REAL*8 TARRIV,Z,C2,R2,ONE/1.D0/,ZERO/0.D0/
      REAL*8 P,P2,GAMS,GAMP
      COMPLEX*16 RS,ZP2,ZGAMS
      COMMON /SOURCE/ A1,A2,DELAY,DISPV
      COMMON /NAG1/   X,Z,C2,R2,TIME,TARRIV
      EXTERNAL RS,SOUR
      CH     = DCOS(Y)
      ...
      ZP2    = DCMPLX(P2,ZERO)
      ZGAMS  = DCMPLX(GAMS,ZERO)
      TRA0SU = -P*GAMS*GAMP*DIMAG( RS(ZP2,ZGAMS) )*SOUR(TIME-TAU)
      RETURN
      END
******---------------------------***************************************
      REAL FUNCTION TRA0SW*8(Y)
      INTEGER DISPV
      REAL*8 TAU,TIME,SOUR,A1,A2,DELAY,X,Y,CH,SH,TEMP
      REAL*8 TARRIV,Z,C2,R2,ONE/1.D0/,ZERO/0.D0/
      REAL*8 P,P2,GAMS,GAMP
      ...
      ZP2    = DCMPLX(P2,ZERO)
      ZGAMS  = DCMPLX(GAMS,ZERO)
      TRA0SW = -DSQRT(C2)*P2*GAMP*DIMAG( RS(ZP2,ZGAMS) )*SOUR(TIME-TAU)
      RETURN
      END
************************************************************************
